...
const SEO = ({ pageName, fallbackTitle, fallbackDesc, image, type = 'website' }) => {
    ...
    const domain = 'https://inkcore.shop'; // Default production domain
    ...
    const title = seo?.meta_title || fallbackTitle || 'Inkcore';
    const description = seo?.meta_description || fallbackDesc || '';
    const metaImage = image || `${domain}/logo/logo.png`;
    ...
    return (
        <Helmet key={pageName + (seo ? '_loaded' : '_loading')}>
            ...
            <meta property="og:site_name" content="Inkcore" />
